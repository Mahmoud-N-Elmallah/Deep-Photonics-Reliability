...
def build_loaders(config: Dict, project_root: Path = None, experiment_type: str = None, is_physics=False):
    ...
    if experiment_type == "tri_channel":
        ...
        fft_mode, input_channels = "dual", 2
    ...
    if is_physics:
        ...
        train_transform = transforms.Compose([
            ...
            v2.RandomChoice([v2.RandomRotation((0, 0)), v2.RandomRotation((90, 90)), v2.RandomRotation((180, 180)), v2.RandomRotation((270, 270))]),
            ...
            FftTransform(width=config['fft_params']['notch_width'], depth=config['fft_params']['notch_depth'], apply_bilateral=config['fft_params']['apply_bilateral'], mode=fft_mode),
        ...
        train_ds = DatasetMaker(config['data_path']['train'], train_transform, project_root, experiment_type)
        val_ds = DatasetMaker(config['data_path']['val'], val_transform, project_root, experiment_type)
        test_ds = DatasetMaker(config['data_path']['test'], val_transform, project_root, experiment_type)
